...
export async function POST(req: Request) {
  try {
    ...
    const { success } = await createTodo(validatedData);
...
export async function PUT(req: Request) {
  try {
    ...
    const { success } = await updateTodo(validatedParams.id, validatedData);
...
export async function DELETE(req: Request) {
  try {
    ...
    const validatedParams = todoIdSchema.parse({ id });
    const { success } = await deleteTodo(validatedParams.id);
