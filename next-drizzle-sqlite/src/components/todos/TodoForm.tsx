...
const TodoForm = ({
  ...
  const handleSubmit = async (data: FormData) => {
    ...
    const pendingTodo: Todo = {
      updatedAt: todo?.updatedAt ?? new Date().toISOString().slice(0, 19).replace("T", " "),
      createdAt: todo?.createdAt ?? new Date().toISOString().slice(0, 19).replace("T", " "),
      id: todo?.id ?? "",
      userId: todo?.userId ?? "",
      ...values,
    };
...
<div>
        <Label
          className={cn(
            "mb-2 inline-block",
            errors?.todoBoolean ? "text-destructive" : "",
          )}
        >
          Todo Boolean
        </Label>
        <br />
        <Checkbox defaultChecked={todo?.todoBoolean} name={'todoBoolean'} className={cn(errors?.todoBoolean ? "ring ring-destructive" : "")} />
        {errors?.todoBoolean ? (
          <p className="text-xs text-destructive mt-2">{errors.todoBoolean[0]}</p>
        ) : (
          <div className="h-6" />
        )}
      </div>
